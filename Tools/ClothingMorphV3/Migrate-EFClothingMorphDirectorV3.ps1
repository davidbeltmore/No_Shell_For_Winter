...
$script = Join-Path $projectRootPath 'Tools\ClothingMorphV3\Migrate-EFClothingMorphDirectorV3.py'
...
$log = Join-Path $logDirectory "EFClothingMorphV3DirectorMigration_$Stamp.log"
...
$env:EF_CLOTHING_V3_MIGRATION_RECEIPT = $receipt
$env:EF_CLOTHING_V3_RESET_RUNTIME_FIT_DEFAULTS = if ($ResetRuntimeFitDefaults) { '1' } else { '0' }
try {
    ...
    Remove-Item Env:EF_CLOTHING_V3_MIGRATION_RECEIPT -ErrorAction SilentlyContinue
    Remove-Item Env:EF_CLOTHING_V3_RESET_RUNTIME_FIT_DEFAULTS -ErrorAction SilentlyContinue
}
...
if (($exitCode -ne 0 -and $exitCode -ne 1) -or
    ...
    $result.status -ne 'UE58_EF_CLOTHING_MORPH_V3_DIRECTOR_MIGRATION_PASS' -or
    -not [bool]$result.success -or
    [int]$result.director_schema_version -ne 4 -or
    $result.director_id -ne 'EFClothingMorphV3' -or
    [int]$result.public_authoring_asset_count -ne 1 -or
    -not [bool]$result.protected_inputs_unchanged) {
    throw "Director V3 migration gate failed: receipt=$receipt log=$log"
}
...
[pscustomobject]@{
    ...
    Garments = @($result.garment_ids) -join ', '
