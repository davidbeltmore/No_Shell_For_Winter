"""Compile EF Clothing Morph V3 bindings without creating garment meshes.

The Director and every Skeletal Mesh are read-only inputs. Only immutable
binding assets and the internal V3 registry are published.
"""
...
DIRECTOR_PATH = os.environ.get(
    "EF_CLOTHING_V3_DIRECTOR",
    "/Game/_Game/Data/EFClothingMorph/DA_EFClothingMorphDirector",
)
COMPATIBILITY_PATH = os.environ.get(
    "EF_CLOTHING_V3_COMPATIBILITY",
    "/Game/DazToUnreal/Multiple/Multiple",
)
RECEIPT_PATH = os.path.realpath(
    os.environ.get(
        "EF_CLOTHING_V3_RECEIPT",
        os.path.join(SAVED_DIR, "ClothingMorphV3QA", "compiler_receipt.json"),
    )
)
OUTPUT_ROOT = "/EFClothingMorph/_Internal/Compiled/V3"
...
def write_receipt(payload):
    if os.path.commonpath((RECEIPT_PATH, SAVED_DIR)).lower() != SAVED_DIR.lower():
        raise RuntimeError("V3 receipt escaped project Saved.")
...
def cleanup_orphan_bindings(active_packages, payload):
    """Delete only unreferenced V3 binding assets through Unreal's editor API."""
    ...
    for candidate in candidates:
        if not candidate.startswith(OUTPUT_ROOT + "/"):
            raise RuntimeError("V3 cleanup candidate escaped the internal output root: " + candidate)
    ...
    if blocked or remaining:
        raise RuntimeError(
            "V3 orphan cleanup was not complete: blocked={} remaining={}".format(
                blocked, remaining
            )
        )
...
def run(payload):
    if os.path.basename(PROJECT_FILE).lower() != "noshellforwinter.uproject":
        raise RuntimeError("V3 compilation is restricted to NoShellForWinter.uproject.")
    ...
    if int(prop(director, "schema_version")) != 4:
        raise RuntimeError("Director is not schema 4 / V3.")
    ...
    if not rows:
        raise RuntimeError("Director has no enabled garments.")
    ids = [str(prop(row, "garment_id")) for row in rows]
    if len(ids) != len(set(ids)):
        raise RuntimeError("Director contains duplicate enabled Garment IDs.")
    ...
    result = unreal.EFClothingFitCompilerLibrary.compile_native_source_catalog_v3(
    ...
    if not bool(prop(result, "success")):
        raise RuntimeError("Native V3 compiler failed: " + payload["native_report"])
    if not package_name(registry).startswith(OUTPUT_ROOT + "/"):
        raise RuntimeError("V3 registry escaped its internal output root.")
    ...
    for row_result in list(prop(result, "rows")):
        ...
        for pair in pairs:
            ...
            pair_payload.append(
                {
                    "garment_lod": int(prop(garment_topology, "lod_index")),
                    "body_lod": int(prop(body_topology, "lod_index")),
                    "garment_vertices": int(prop(garment_topology, "render_vertex_count")),
        ...
        payload["rows"].append(
            {
                ...
                "source_mesh": package_name(prop(binding, "source_garment")) if binding else "",
                "fitted_mesh": package_name(prop(binding, "fitted_garment")) if binding else "",
                "valid": row_valid,
    ...
    payload["catalog_equality_gate"] = (
        ...
        and expected == len(payload["rows"])
        and expected == valid
        and expected == len(native_bindings)
        and len(profiles) == 0
    )
    if not payload["catalog_equality_gate"]:
        raise RuntimeError("V3 enabled/compiled/binding equality gate failed.")
    if not payload["protected_inputs_unchanged"]:
        raise RuntimeError("A protected mesh, skeleton, Player or Director changed during V3 binding compilation.")
...
def main():
    payload = {
        "schema_version": 1,
        "compiler_version": 27,
        "binding_schema_version": 7,
        "generated_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
        "status": "UE58_EF_CLOTHING_MORPH_V3_CATALOG_COMPILE_FAIL",
        "success": False,
    ...
    try:
        ...
        payload["status"] = "UE58_EF_CLOTHING_MORPH_V3_CATALOG_COMPILE_PASS"
    ...
    unreal.log("EF_CLOTHING_MORPH_V3_CATALOG_RECEIPT=" + RECEIPT_PATH)
