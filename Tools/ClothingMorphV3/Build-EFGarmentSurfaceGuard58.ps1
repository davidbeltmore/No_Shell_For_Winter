...
$editors = @(Get-Process -ErrorAction SilentlyContinue | Where-Object {
    $_.ProcessName -eq 'UnrealEditor' -or $_.ProcessName -eq 'UnrealEditor-Cmd'
})
if ($editors.Count -ne 0) {
    throw 'Close every Unreal Editor process before rebuilding the Surface Guard graph.'
}
...
if (-not $process.HasExited) {
    ...
    throw "Surface Guard graph build exceeded $TimeoutSeconds seconds. Log: $log"
}
...
if (($process.ExitCode -ne 0 -and $process.ExitCode -ne 1) -or
    -not $commandletSucceeded -or $commandletFailed) {
    throw "Surface Guard graph build failed: exit=$($process.ExitCode) log=$log"
}

$receipt = Get-ChildItem -LiteralPath $receiptDirectory `
    -Filter 'DG_EFGarmentSurfaceConstraint_*.json' -File |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1
if (-not $receipt) {
    throw 'Surface Guard graph builder produced no receipt.'
}
...
[pscustomobject]@{
    Status = 'UE58_EF_CLOTHING_MORPH_V3_SURFACE_GUARD_PASS'
