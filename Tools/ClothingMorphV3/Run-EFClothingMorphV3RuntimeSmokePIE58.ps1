...
$PythonScript = Join-Path $ProjectRoot 'Tools\ClothingMorphV3\Validate-EFClothingMorphV3RuntimeSmokePIE58.py'
...
# Static safety gates keep this V3 harness source-first and prevent accidental
...
if ($PythonSource -match '(?i)\.set_skeletal_mesh(?:_asset)?\s*\(') {
    throw 'The V3 smoke harness must never assign a Skeletal Mesh directly.'
}
if ($PythonSource -match '(?i)(compile_native_source_catalog_v3|compile_all_garments|compile_catalog)\s*\(') {
    throw 'The V3 runtime smoke harness must never invoke a clothing compiler.'
}
if ($PythonSource -match '(?i)(EditorAssetLibrary\.)?save_asset\s*\(') {
    throw 'The V3 runtime smoke harness must never save an Unreal asset.'
}
if ($PythonSource.IndexOf('call(STATE.interaction, "interact", "")', [System.StringComparison]::Ordinal) -lt 0) {
    throw 'The V3 smoke harness no longer invokes the real ACF Interact route.'
}
foreach ($forbiddenToken in @(
    ...
    if ($PythonSource.IndexOf($forbiddenToken, [System.StringComparison]::Ordinal) -ge 0) {
        throw "The V3 smoke harness contains forbidden V26 readback/runtime token: $forbiddenToken"
    }
}
foreach ($requiredToken in @(
    ...
    if ($PythonSource.IndexOf($requiredToken, [System.StringComparison]::Ordinal) -lt 0) {
        throw "The V3 smoke harness is missing required contract token: $requiredToken"
    }
}

$RunId = Get-Date -Format 'yyyyMMdd_HHmmss'
$RunDir = Join-Path $ProjectRoot "Saved\ClothingMorphV3QA\RuntimeSmoke_$RunId"
$RuntimeResultPath = Join-Path $RunDir 'RuntimeResult.json'
$ProgressLog = Join-Path $RunDir 'Progress.log'
$EditorLog = Join-Path $RunDir 'UnrealEditor_EFClothingMorphV3RuntimeSmoke.log'
...
function Get-ProtectedAssetState {
    ...
    $v3Root = Join-Path $ProjectRoot 'Plugins\EFClothingMorph\Content\_Internal\Compiled\V3'
    if (-not (Test-Path -LiteralPath $v3Root -PathType Container)) {
        throw "V3 compiled root is missing: $v3Root"
    }
    $relativePaths += @(
        Get-ChildItem -LiteralPath $v3Root -File -Filter '*.uasset' -Recurse |
...
$environmentValues = [ordered]@{
    ...
    CODEX_EF_CLOTHING_V3_QA_DIR = $RunDir
    CODEX_EF_CLOTHING_V3_QA_RESULT = $RuntimeResultPath
    CODEX_EF_CLOTHING_V3_QA_MAP = $MapPath
    CODEX_EF_CLOTHING_V3_QA_TIMEOUT = [string]($TimeoutSeconds - 60)
}
...
try {
    ...
    if ($existingEditors.Count -gt 0) {
        throw "An Unreal Editor already targets this project; close it before the isolated V3 smoke run. PID(s): $($existingEditors.ProcessId -join ', ')"
    }
    ...
    if ($additionalArguments -match '(?i)nullrhi') {
        throw 'The V3 runtime smoke must never use NullRHI.'
    }
    ...
    if (-not $launcher.HasExited) {
        ...
        throw "V3 runtime smoke exceeded $TimeoutSeconds seconds."
    }
    ...
    if ([string]$runtime.status -ne 'UE58_EF_CLOTHING_MORPH_V3_RUNTIME_SMOKE_PASS') {
        throw "V3 runtime smoke failed: $($runtime.failure)"
    }
    if (
        [int]$runtime.expected_compiler_version -ne 27 -or
        [int]$runtime.expected_binding_schema -ne 7 -or
        ...
        [string]$runtime.gpu_readback.status -ne 'NOT_USED_V3_SMOKE' -or
        [bool]$runtime.gpu_readback.v26_readback_dependency -or
        -not [bool]$runtime.no_assets_saved
    ) {
        throw 'V3 runtime result is missing a required source-first/HUB/no-readback gate.'
    }
    if (
        [int]$runtime.catalog.enabled_row_count -lt 1 -or
        [int]$runtime.catalog.enabled_row_count -ne [int]$runtime.catalog.native_binding_count -or
        [int]$runtime.catalog.generated_profile_count -ne 0 -or
        [int]$runtime.catalog.enabled_row_count -ne @($runtime.rows_tested).Count
    ) {
        throw 'V3 runtime catalog does not equal its binding-only registry/test coverage.'
    }
    ...
    foreach ($runtimeRow in @($runtime.rows_tested)) {
        ...
        if (
            ...
            throw "V3 exact SourceGarment gate failed for $($runtimeRow.row_name)."
        }
        if (
            ...
            throw "V3 real ACF acquisition/GUID gate failed for $($runtimeRow.row_name)."
        }
        if (
            ...
            throw "V3 live offset sequence failed for $($runtimeRow.row_name)."
        }
        if (
            ...
            throw "V3 idle/locomotion smoke failed for $($runtimeRow.row_name)."
        }
        if (
            ...
            throw "V3 three-cycle ACF smoke failed for $($runtimeRow.row_name)."
        }
        ...
        foreach ($checkpoint in @($runtimeRow.runtime_checks)) {
            if (
                ...
                throw "Certified V3 runtime checkpoint is not visible source-first Ready: $($runtimeRow.row_name)/$($checkpoint.checkpoint)"
            }
        }
        ...
        if (
            ...
            throw "V3 initial Director-authored clearance/inflate snapshot failed for $($runtimeRow.row_name)."
        }
    }
    ...
    $expectedScreenshotCount = 4 * [int]$runtime.catalog.enabled_row_count
    if ($screenRows.Count -ne $expectedScreenshotCount) {
        throw "Expected $expectedScreenshotCount V3 gameplay screenshots; found $($screenRows.Count)."
    }
    ...
    $criticalPatterns = @(
        ...
        '[EFClothingMorphV3RuntimeSmoke] finished success=False'
    )
    ...
    if (@($summary.critical_log_matches).Count -gt 0) {
        throw "Critical V3 runtime log signatures were observed: $(@($summary.critical_log_matches).Count)"
    }

    $summary.cleanup = 'CLEAN_EDITOR_SELF_EXIT'
    $summary.status = 'UE58_EF_CLOTHING_MORPH_V3_RUNTIME_SMOKE_PASS'
}
catch {
    $failureException = $_
    $summary.status = 'UE58_EF_CLOTHING_MORPH_V3_RUNTIME_SMOKE_FAIL'
    ...
    try {
        ...
        if (-not $summary.protected_assets_unchanged -or -not $summary.git_state_unchanged) {
            throw 'Worktree or protected Unreal assets changed during V3 runtime smoke.'
        }
    }
    catch {
...
Write-Host "V3 runtime smoke summary: $SummaryPath"
