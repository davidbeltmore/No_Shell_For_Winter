"""Migrate eight Calysto demo maps and exact OFPA package families via UE 5.7.

Required environment:
  CODEX_CALYSTO_DEMO_MAPS_RECEIPT
  CODEX_CALYSTO_DEMO_MAPS_VALIDATION_EVIDENCE
  CODEX_CALYSTO_DEMO_MAPS_TARGET_ROOT
  CODEX_CALYSTO_DEMO_MAPS_MIGRATION_EVIDENCE
  CODEX_CALYSTO_DEMO_MAPS_EXPECTED_HARNESS_CONTENT
"""
...
RECEIPT_VALUE = os.environ.get("CODEX_CALYSTO_DEMO_MAPS_RECEIPT", "").strip()
VALIDATION_VALUE = os.environ.get(
    "CODEX_CALYSTO_DEMO_MAPS_VALIDATION_EVIDENCE", ""
).strip()
TARGET_ROOT_VALUE = os.environ.get("CODEX_CALYSTO_DEMO_MAPS_TARGET_ROOT", "").strip()
...
def fail(message):
    unreal.log_error("CODEX_CALYSTO_DEMO_MAPS57_MIGRATION_FAIL: " + message)
...
if not all(
    ...
    fail("All documented CalystoDemoMaps migration variables are required")
...
if receipt.get("status") != "ISOLATED_CALYSTO_DEMO_MAPS57_HARNESS_PASS":
    fail("Harness receipt is not PASS")
if validation.get("status") != "UE57_CALYSTO_DEMO_MAPS_READ_ONLY_LOAD_PASS":
...
if validation.get("map_count") != 8 or validation.get("map_save_requested") is not False:
    fail("Validation evidence does not prove eight read-only map loads")
...
unreal.log(
    "CODEX_CALYSTO_DEMO_MAPS57_BEGIN: seeds={} collisions={}".format(
...
for package_row in package_rows:
    ...
    for row in package_row.get("files", []):
        ...
        if target_length <= 0:
            fail("Migrated target package is empty: " + package)
        package_output.append(
            {
                ...
                "source_sha256": str(row["sha256"]).upper(),
                "assettools_reserialized": (
...
for package_row in package_rows:
    ...
    for row in package_row.get("files", []):
        ...
        for label in ("source", "staged"):
            path = os.path.realpath(str(row.get(label, "")))
            if not os.path.isfile(path):
                fail("{} package vanished during migration: {}".format(label, package))
            actual = (os.path.getsize(path), sha256(path))
            if actual != expected:
                fail("{} package changed during migration: {}".format(label, package))
...
payload = {
    ...
    "status": "ASSETTOOLS_EXACT_CALYSTO_DEMO_MAPS57_MIGRATION_PASS",
    ...
    "assettools_reserialization_allowed": True,
...
unreal.log("CODEX_CALYSTO_DEMO_MAPS57_MIGRATION_PASS: " + evidence_path)
