"""Read-only UE 5.7 load gate for the eight Calysto demo maps harness.
...
def fail(message):
    unreal.log_error("CODEX_CALYSTO_DEMO_MAPS57_VALIDATION_FAIL: " + message)
...
if not RECEIPT_VALUE or not OUTPUT_VALUE or not EXPECTED_HARNESS_VALUE:
    fail("All documented CalystoDemoMaps validation variables are required")
...
if len(maps) != 8 or map_packages != EXPECTED_MAPS:
...
payload = {
    ...
    "status": "UE57_CALYSTO_DEMO_MAPS_READ_ONLY_LOAD_PASS",
...
unreal.log("CODEX_CALYSTO_DEMO_MAPS57_VALIDATION_PASS: " + output_path)
