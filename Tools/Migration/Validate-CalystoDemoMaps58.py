"""Fresh UE 5.8 read-only integrity gate for migrated Calysto demo maps.

Required environment:
  CODEX_CALYSTO_DEMO_MAPS_MIGRATION_EVIDENCE
  CODEX_CALYSTO_DEMO_MAPS_58_EVIDENCE
"""
...
OUTPUT_VALUE = os.environ.get("CODEX_CALYSTO_DEMO_MAPS_58_EVIDENCE", "").strip()
EXPECTED_MAPS = (
    "/Game/Calysto/Dungeon/Demo/L_MassiveDungeon_Demo",
    "/Game/Calysto/Dungeon/Demo/L_MassiveDungeonSubstrate_Demo",
    "/Game/Calysto/Dungeon/Demo/LevelInstance/L_Bones",
    "/Game/Calysto/Dungeon/Demo/LevelInstance/L_Forge",
    "/Game/Calysto/Dungeon/Demo/LevelInstance/L_LevelInstanceBuilder",
    "/Game/Calysto/Dungeon/Demo/LevelInstance/L_Statue",
    "/Game/Calysto/Dungeon/Demo/LevelInstance/L_Table",
    "/Game/Calysto/Dungeon/Demo/LevelInstance/L_WeaponShelf",
)


def fail(message):
    unreal.log_error("CODEX_CALYSTO_DEMO_MAPS58_VALIDATION_FAIL: " + message)
...
if not MIGRATION_VALUE or not OUTPUT_VALUE:
    fail("Both documented CalystoDemoMaps UE 5.8 variables are required")
...
if migration.get("status") != "ASSETTOOLS_EXACT_CALYSTO_DEMO_MAPS57_MIGRATION_PASS":
...
validation57_path = os.path.realpath(str(migration.get("validation", "")))
if not is_under(validation57_path, os.path.join(project_root, "Saved", "Migration")):
    fail("UE 5.7 validation evidence escapes target Saved/Migration")
if not os.path.isfile(validation57_path):
    fail("UE 5.7 validation evidence is absent")
if sha256(validation57_path) != str(migration.get("validation_sha256", "")).upper():
    fail("UE 5.7 validation evidence hash differs from the migration chain")
with open(validation57_path, "r", encoding="utf-8-sig") as handle:
    validation57 = json.load(handle)
if validation57.get("status") != "UE57_CALYSTO_DEMO_MAPS_READ_ONLY_LOAD_PASS":
    fail("UE 5.7 read-only map validation is not PASS")
expected57_maps = {
    str(row.get("package", "")): row for row in validation57.get("maps", [])
}
if set(expected57_maps) != set(EXPECTED_MAPS):
    fail("UE 5.7 validation map set differs")
...
if not set(EXPECTED_MAPS).issubset(package_names):
    fail("A required Calysto demo map is absent from migration evidence")
...
for row in map_results:
    baseline = expected57_maps[row["package"]]
    if row["actor_count"] != int(baseline.get("actor_count", -1)):
        fail("Actor count changed across UE 5.7 -> 5.8 for " + row["package"])
    if row["actor_classes"] != list(baseline.get("actor_classes", [])):
        fail("Actor class inventory changed across UE 5.7 -> 5.8 for " + row["package"])
    if row["world_partitioned"] != bool(baseline.get("world_partitioned", False)):
        fail("World partition state changed across UE 5.7 -> 5.8 for " + row["package"])
...
payload = {
    ...
    "status": (
        "UE58_CALYSTO_DEMO_MAPS_FRESH_READ_ONLY_LOAD_PASS"
        if not unresolved_game_dependencies
        else "UE58_CALYSTO_DEMO_MAPS_DEPENDENCY_GAP"
    ),
    ...
    "validation57_evidence": validation57_path,
    "validation57_evidence_sha256": sha256(validation57_path),
...
if unresolved_game_dependencies:
    fail(
        "Calysto demo maps retain unresolved /Game dependencies; evidence: "
        + output_path
    )
unreal.log("CODEX_CALYSTO_DEMO_MAPS58_VALIDATION_PASS: " + output_path)
