...
function Assert-True {
    ...
    if (-not $Condition) { throw "CALYSTO_DEMO_MAPS57_HARNESS_FAIL: $Message" }
}
...
$runsRoot = Join-Path $target 'Saved\Migration\Phase4\CalystoDemoMaps\Runs'
$runRoot = Join-Path $runsRoot $RunId
$harnessRoot = Join-Path $runRoot 'Harness57'
$harnessContent = Join-Path $harnessRoot 'Content'
$harnessProject = Join-Path $harnessRoot 'CalystoDemoMaps57Harness.uproject'
$harnessConfig = Join-Path $harnessRoot 'Config\DefaultEngine.ini'
$receiptPath = Join-Path $runRoot 'CalystoDemoMaps57HarnessReceipt.json'
...
$mapSpecs = @(
    [pscustomobject]@{ Package = '/Game/Calysto/Dungeon/Demo/L_MassiveDungeon_Demo'; Relative = 'Calysto\Dungeon\Demo\L_MassiveDungeon_Demo.umap' },
    [pscustomobject]@{ Package = '/Game/Calysto/Dungeon/Demo/L_MassiveDungeonSubstrate_Demo'; Relative = 'Calysto\Dungeon\Demo\L_MassiveDungeonSubstrate_Demo.umap' },
    [pscustomobject]@{ Package = '/Game/Calysto/Dungeon/Demo/LevelInstance/L_Bones'; Relative = 'Calysto\Dungeon\Demo\LevelInstance\L_Bones.umap' },
    [pscustomobject]@{ Package = '/Game/Calysto/Dungeon/Demo/LevelInstance/L_Forge'; Relative = 'Calysto\Dungeon\Demo\LevelInstance\L_Forge.umap' },
    [pscustomobject]@{ Package = '/Game/Calysto/Dungeon/Demo/LevelInstance/L_LevelInstanceBuilder'; Relative = 'Calysto\Dungeon\Demo\LevelInstance\L_LevelInstanceBuilder.umap' },
    [pscustomobject]@{ Package = '/Game/Calysto/Dungeon/Demo/LevelInstance/L_Statue'; Relative = 'Calysto\Dungeon\Demo\LevelInstance\L_Statue.umap' },
    [pscustomobject]@{ Package = '/Game/Calysto/Dungeon/Demo/LevelInstance/L_Table'; Relative = 'Calysto\Dungeon\Demo\LevelInstance\L_Table.umap' },
    [pscustomobject]@{ Package = '/Game/Calysto/Dungeon/Demo/LevelInstance/L_WeaponShelf'; Relative = 'Calysto\Dungeon\Demo\LevelInstance\L_WeaponShelf.umap' }
)
...
function Add-SupportPackage {
    ...
    if ($migrationPackages.Contains($Package)) { return }
...
# Stage every non-map Calysto package from UE 5.7 for load support only. This
# closes opaque Blueprint/PCG references without widening the live migration set.
$calystoSupportRoot = Join-Path $sourceContent 'Calysto'
$calystoSupportPrimaries = @(
    Get-ChildItem -LiteralPath $calystoSupportRoot -Recurse -File -Force -Filter '*.uasset' |
        Sort-Object FullName
)
foreach ($file in $calystoSupportPrimaries) {
    $relative = $file.FullName.Substring($sourceContent.Length).TrimStart('\')
    $package = Convert-RelativeFileToPackage -RelativeFile $relative
    if (-not $migrationPackages.Contains($package)) {
        Add-SupportPackage -Package $package -OwnerMap 'CALYSTO_DEMO_SHARED_LOAD_SUPPORT'
    }
}

Assert-True ($mapRows.Count -eq 8) 'Calysto demo map count differs from eight.'
Assert-True ([int](($mapRows | Measure-Object -Property external_actor_package_count -Sum).Sum) -eq 50) 'Expected exactly 50 OFPA external actor packages.'
Assert-True ([int](($mapRows | Measure-Object -Property external_object_package_count -Sum).Sum) -eq 0) 'Unexpected external object packages entered the batch.'
Assert-True (@($mapRows | Where-Object built_data_present).Count -eq 0) 'Unexpected BuiltData package entered the batch.'
Assert-True ($migrationPackages.Count -eq 58) 'Expected exactly 8 maps plus 50 OFPA packages.'
...
[ordered]@{
    ...
    Description = 'Detached UE 5.7 harness for the eight Calysto demo maps and their exact OFPA/external package sets.'
...
Assert-True (@($migrationPackageRows | Where-Object target_collision_at_stage).Count -eq 0) 'A Calysto demo map or OFPA target collision already exists.'

$payload = [ordered]@{
    schema_version = 1
    generated_utc = [DateTime]::UtcNow.ToString('o')
    status = 'ISOLATED_CALYSTO_DEMO_MAPS57_HARNESS_PASS'
    run_id = $RunId
    source_root = $source
    target_root = $target
    ...
    migration_policy = 'Only the eight Calysto demo maps plus exact BuiltData/__ExternalActors__/__ExternalObjects__ package families are migration seeds. Direct /Game dependencies are staged for UE 5.7 load only. AssetTools must run with ignore_dependencies=true and conflict=SKIP.'
}
$payload | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $receiptPath -Encoding UTF8

Write-Host "CALYSTO_DEMO_MAPS57_HARNESS_PASS: $harnessProject"
