...
$runDirectory = Join-Path $ProjectRoot "Saved\ClothingMorphV4QA\PackagedStartup_$Stamp"
...
$summary = [ordered]@{
    schema_version = 1
    status = 'UE58_EF_CLOTHING_MORPH_V4_PACKAGED_STARTUP_FAIL'
...
try {
    ...
    $summary.status = 'UE58_EF_CLOTHING_MORPH_V4_PACKAGED_STARTUP_PASS'
}
finally {
...
if ($summary.status -ne 'UE58_EF_CLOTHING_MORPH_V4_PACKAGED_STARTUP_PASS') {
    throw "V4 packaged startup QA failed. Summary: $summaryPath"
}
Write-Output "V4 packaged startup QA: PASS"
