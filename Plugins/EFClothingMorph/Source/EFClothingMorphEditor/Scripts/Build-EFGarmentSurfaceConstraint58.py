"""Build and validate the canonical EF Clothing Morph V4 Optimus graph.
...
def main() -> None:
    ...
    receipt_directory = os.path.join(
        unreal.Paths.project_saved_dir(), "ClothingMorphV4QA", "GraphBuilder"
    )
    ...
    with open(receipt_path, "w", encoding="utf-8") as receipt_file:
        json.dump(
            {
                "schema": "EFClothingMorph.SurfaceGraph.35.0",
                "asset": (
    ...
    unreal.log(f"EF Clothing Morph V4 graph receipt: {receipt_path}")
